...
/*!50001 VIEW `report` AS select `d`.`OrderID` AS `OrderID`,round((sum((`d`.`Quantity` * `p`.`Price`)) / sum(`d`.`Quantity`)),1) AS `price` from (`orderdetails` `d` join `products` `p`) where (`d`.`ProductID` = `p`.`ProductID`) group by `d`.`OrderID` */;
